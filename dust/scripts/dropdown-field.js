(function(global, factory) {
    ...
    function DropdownField(target, fieldLabel, placeholder, tabindex, ID, opts) {
        ...
        function listSelectionChange(list, selectClass, down, currentSelection, listLength) {
        ...
        function listSelectionChangeScroll(list, selectClass, down, currentSelection, listLength, scrollAmount, listSize) {
            if (listLength > 0) {
                if (currentSelection === -1) {
                    if (down) {
                        list.scrollTo(0, 0);
            ...
            switch (keyCodes[e.keyCode]) {
              case "up":
                if (!ddVisible && arrowKeysNoDropdown !== 2) {
                    ...
                    let val = listSelectionChange(elUL, "selected", false, index, selectionLength);
                    if (val) {
                        elInput.value = val;
                        if (elAutocomplete) elAutocomplete.classList.remove("isvisible");
                        if (settings.onChange) settings.onChange();
                    ...
                    if (ddVisible) {
                        listSelectionChangeScroll(elUL, "selected", false, index, selectionLength, lineHeight, maxHeight);
                    }
                }
                break;
              ...
              case "down":
                if (!ddVisible && arrowKeysNoDropdown !== 2) {
                    ...
                    let val = listSelectionChange(elUL, "selected", true, index, selectionLength);
                    if (val) {
                        elInput.value = val;
                        if (elAutocomplete) elAutocomplete.classList.remove("isvisible");
                        if (settings.onChange) settings.onChange();
                    ...
                    if (ddVisible) {
                        listSelectionChangeScroll(elUL, "selected", true, index, selectionLength, lineHeight, maxHeight);
                    }
                }
                break;
